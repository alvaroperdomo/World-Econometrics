...
El rango de $\mathbf{\pi}$ es igual al número de vectores de cointegración. Por analogía con el caso univariado, si $\mathbf{\pi}$ tiene solo ceros, de modo que el $rango(\pi)=0$, entonces todas las secuencias de { $x_{it}$ }, a pesar de tener raíz unitaria, no hay una combinación lineal de los procesos { $x_{it}$ } que sea estacionaria, las variables no se cointegran. 

Descartando la presencia de raíces características mayores que $1$ y si el $rango(\pi)=n$, $\mathbf{\Delta x_t=\pi x_{t-1} + \varepsilon_t}$ es un sistema convergente de ecuaciones en diferencias, de modo que todas las variables son estacionarias.
