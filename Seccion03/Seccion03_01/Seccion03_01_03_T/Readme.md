...
Siguiendo la notación de la sección 3.1.1. en un **$VAR(2)$ estandar** se estiman $9$ parámetros [ $6$ coeficientes ($a_{10}$, $a_{20}$, $a_{11}$, $a_{12}$, $a_{21}$ y $a_{22}$), $\sigma_{e1t}^2$, $\sigma_{e2t}^2$ y $\sigma_{e1t,e2t}$]. Sin embargo, en un **$VAR(2) estructural** se necesitan $10$ parámetros [ $2$ interceptos ($a_{10}$ y $a_{20}$), $4$ coeficientes autorregresivos ($\gamma_{11}$, $\gamma_{12}$, $\gamma_{21}$ y $\gamma_{22}$), $2$ coeficientes de retroalimentación ($b_{12}$, $b_{21}$), $\sigma_y$ y $\sigma_z$] 
