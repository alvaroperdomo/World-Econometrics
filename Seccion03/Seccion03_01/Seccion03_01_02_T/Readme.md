...
Para determinar el número de rezagos, mucho más utilizadas que las pruebas de razón de verosimilitud, son las generalizaciones multivariadas del $\text{Criterio de Información de Akaike}$ y el $\text{Criterio Bayesiano de Schwartz}$:
...
Si se cumple la condición de estabilidad, podemos escribir la solución particular de $x_t$ como modelo de Vectores de Media Movil ($VMA$ por sus siglas en inglés) representado por $\mathbf{\mu+\displaystyle\sum_{i=0}^\infty A_1^i e_{t-i}}$
